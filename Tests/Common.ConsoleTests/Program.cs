    ...
    internal class Program
    {
        ...
        private static void DoSpeedTest()
        {
            ...
            for (var i = 0;i < 6;++i) {
                ...
                Console.WriteLine("{0,-5} times using {1,25} => {2} ms", iterations, "UseStringVerified",
                                  TimeLoop(iterations, () => { testObject.UseStringVerified = "XX"; }));
                Console.WriteLine("{0,-5} times using {1,25} => {2} ms", iterations, "UseMethodBase", TimeLoop(iterations, () => { testObject.UseMethodBase = "XX"; }));
                Console.WriteLine("{0,-5} times using {1,25} => {2} ms", iterations, "UseMethodBaseVerified",
                                  TimeLoop(iterations, () => { testObject.UseMethodBaseVerified = "XX"; }));
                Console.WriteLine("{0,-5} times using {1,25} => {2} ms", iterations, "UseExpression", TimeLoop(iterations, () => { testObject.UseExpression = "XX"; }));
                Console.WriteLine("{0,-5} times using {1,25} => {2} ms", iterations, "UseExpressionVerified",
                                  TimeLoop(iterations, () => { testObject.UseExpressionVerified = "XX"; }));
                Console.WriteLine();
            }
        }
